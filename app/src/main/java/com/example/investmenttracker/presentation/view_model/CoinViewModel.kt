...
class CoinViewModel(
    ...
    private val coinSearched: MutableLiveData<Resource<APIResponse>> = MutableLiveData()

    fun getCoin(name: String, currency: String) = viewModelScope.launch(Dispatchers.IO) {
        coinSearched.postValue(Resource.Loading())
        try {
            if (isNetworkAvailable(app)){
                val apiResult = getCoinUseCase.execute(name, currency)
